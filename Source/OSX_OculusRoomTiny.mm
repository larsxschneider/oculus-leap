    ...
    ControlDown(false)
{
    ...
    SConfig.SetIPD(0.0665);
    LeapPinterLengthInMM = 300.0f;
}
...
int OculusRoomTinyApp::OnStartup(const char* args)
{
    ...
    pLeapController = new Leap::Controller();
    pLeapController->addListener(*this);

    return 0;
}


void OculusRoomTinyApp::onFrame(const Leap::Controller& leapController)
{
    const Leap::Frame frame = leapController.frame();

    if (frame.tools().count() >= 2)
    {      
        const Leap::Vector leftRift = frame.tools().leftmost().tipPosition()
                                    - frame.tools().leftmost().direction() * LeapPinterLengthInMM;
        const Leap::Vector rightRift = frame.tools().rightmost().tipPosition()
                                     - frame.tools().rightmost().direction() * LeapPinterLengthInMM;
        const Leap::Vector centerRift = 0.5f * (leftRift + rightRift);
        const Leap::Vector centerRiftInMeter = 0.01 * centerRift;

        if (LeapInitilized)
        {
            Vector3f local;
            local.x = 0.0f + centerRiftInMeter.x + LeapOffset.x;
            local.y = 1.6f + centerRiftInMeter.y + LeapOffset.y;
            local.z = 0.0f + centerRiftInMeter.z + LeapOffset.z;

            Matrix4f yawRotate = Matrix4f::RotationY(EyeYaw);
            EyePos = yawRotate.Transform(local);
        }
        else
        {
            LeapOffset = -centerRiftInMeter;
            LeapInitilized = true;
        }
    }
}
